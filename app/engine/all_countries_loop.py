...
def read_benefit_params(country, filepath):
    ...
    child_death = np.array(
        child_lives_saved.loc[child_lives_saved["Country"] == country, 2022:2030]
    )
    child_death = np.nan_to_num(child_death).flatten()
    neo_death = np.array(
        neonatal_lives_saved.loc[neonatal_lives_saved["Country"] == country, 2022:2030]
    )
    neo_death = np.nan_to_num(neo_death).flatten()
    stillbirth = np.array(
        stillbirths_averted.loc[stillbirths_averted["Country"] == country, 2022:2030]
    )
    stillbirth = np.nan_to_num(stillbirth).flatten()
    preg_avert = np.array(
        unintended_preg_avert.loc[
            unintended_preg_avert["Country"] == country, 2022:2030
        ]
    )
    preg_avert = np.nan_to_num(preg_avert).flatten()

    # FP deaths averted
    fp_mat_death = np.array(
        fp_maternal_lives_saved.loc[
            fp_maternal_lives_saved["Country"] == country, 2022:2030
        ]
    )
    ...
    neo_morb = {}
    mat_morb0 = np.array(
        maternal_morbidities_averted.loc[
            maternal_morbidities_averted["Country"] == country, 2022:2030
        ]
    )
    mat_morb["YLD"] = np.nan_to_num(mat_morb0).flatten()
    child_morb0 = np.array(
        wasting_averted.loc[wasting_averted["Country"] == country, 2022:2030]
    )
    child_morb["Wasting"] = np.nan_to_num(child_morb0).flatten()
    child_morb0 = np.array(
        stunting_averted.loc[stunting_averted["Country"] == country, 2022:2030]
    )
    child_morb["Stunting"] = np.nan_to_num(child_morb0).flatten()
    neo_morb0 = np.array(
        neo_morbidities_averted.loc[
            neo_morbidities_averted["Country"] == country, 2022:2030
        ]
    )
...
def total_econ_benefits(
    ...
    dalys_mat_death = calc_daly_annual(
        population_mat_death, pars_country, start_year=start_year, end_year=end_year
    )
    dalys_child_death = calc_daly_annual(
        population_child_death, pars_country, start_year=start_year, end_year=end_year
    )
    dalys_neo_death = calc_daly_annual(
        population_neo_death, pars_country, start_year=start_year, end_year=end_year
    )
    dalys_stillbirth = calc_daly_annual(
        population_stillbirth, pars_country, start_year=start_year, end_year=end_year
    )
    ...
    work_benefits_mat_mort = calc_work_benefits(
        population_mat_death,
        pars_country,
        work_participation=pars_country[
    ...
    work_benefits_preg_avert = calc_work_benefits(
        population_preg_averted_labour,
        pars_country,
        work_participation=pars_country[
            "Proportion of women who participate in workforce"
        ][0],
        prop_year=pars_country["Maternal time out of workforce"][0],
        start_year=start_year,
        end_year=end_year,
    )
    work_benefits_child_mort = calc_work_benefits(
        population_child_death,
        pars_country,
        work_participation=pars_country["Workforce participation"][0],
        prop_year=1,
        start_year=start_year,
        end_year=end_year,
    )
    work_benefits_neo_mort = calc_work_benefits(
        population_neo_death,
        pars_country,
        work_participation=pars_country["Workforce participation"][0],
        prop_year=1,
    ...
    work_benefits_stillbirth = calc_work_benefits(
        population_stillbirth,
        pars_country,
        work_participation=pars_country["Workforce participation"][0],
        prop_year=1,
    ...
    work_econ_benefits = (
        work_benefits_mat_mort
        + work_benefits_preg_avert
        + work_benefits_child_mort
        + work_benefits_neo_mort
        + work_benefits_stillbirth
    )

    """Education benefits"""
    # Calculate education benefits
    edu_econ_benefits = calc_education_benefits(
        population_preg_averted_education,
        pars_country,
        end_year=end_year,
        start_year=start_year,
    ...
    if len(social_econ_benefits) < len(edu_econ_benefits):
        social_econ_benefits = np.append(
            social_econ_benefits,
            np.zeros(
                [
                    len(edu_econ_benefits) - len(social_econ_benefits),
                    len(social_econ_benefits),
                ]
            ),
            axis=0,
        )
    elif len(social_econ_benefits) > len(
        edu_econ_benefits
    ):  # If necessary remove start year
        social_econ_benefits = np.delete(social_econ_benefits, 0, 0)

    """Economic benefits output """
    # Total economic benefit from scale up
    sum_econ_benefits = (
        sum(social_econ_benefits) + sum(work_econ_benefits) + sum(edu_econ_benefits)
    )
    ...
    child_death_benefits = sum(dalys_child_death) + sum(
        work_benefits_child_mort
    )  # Including HIV disabilities
    ...
    return (
        sum_econ_benefits,
        maternal_death_benefits,
        neonatal_death_benefits,
        child_death_benefits,
        stillbirth_benefits,
        teen_preg_avert_edu_benefits,
        preg_avert_work_benefits,
        work_benefits_tot,
        edu_benefits_tot,
        social_benefits_tot,
        BCR,
...
def run_bcr_script(
    ...
    for r, region in enumerate(region_list):
        for p, package in enumerate([""]):
            ...
            label_data = {
                "Year": [
                    region + package + " " + str(year) for year in label_data["Year"]
                ]
            }
            total_data = {
                "Neonatal YLLs": dalys[0],
                "Child YLLs": dalys[1],
                "Maternal YLLs": dalys[2],
                "Stillbirth YLLs": dalys[3],
                "Neonatal DALYs": dalys[4],
                "Child DALYs": dalys[5],
                "Maternal DALYs": dalys[6],
                "Total YLLs": dalys[7],
                "Total DALYs": dalys[8],
                "Total": dalys[9],
            }
            ...
            print(
                "Education BCR: %0.1f percent of total benefits" % standard_benefits[12]
            )
    ...
    if include_all_countries:
        totals = (
            main_results_df.groupby(["Benefit cause"])
        ...
        totals.loc[totals["Benefit cause"] == "Total BCR", ["Outcomes"]] = (
        ...
        totals.loc[
            totals["Benefit cause"] == "Education % of benefits", ["Outcomes"]
        ] = (
