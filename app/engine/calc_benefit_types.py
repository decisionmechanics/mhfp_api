...
def calc_education_benefits(
    input_population, pars_country, start_year=2022, end_year=2030
):
    """
    ...
    earn_mult = (
        ...
        * pars_country["Education benefit"][0]
    )
    ...
    for year in years:
        edu_workers_earnings[year] = (
            np.sum(input_population[year, 18:64] * earn_mult)
            ...
            * (1 / (1 + pars_country["Discounting"][0]) ** (year + 2))
        )  # 2020 is year 0

    return edu_workers_earnings


def calc_work_benefits(
    input_population,
    pars_country,
    work_participation,
    prop_year=1,
    ...
    for year in years:
        workers_earnings[year] = (
            sum(input_population[year, 18:64])
            * work_participation
            * pars_country["Average annual salary"][0]
            * (1 + pars_country["Annual GDP growth"][0]) ** (year + 2)
            * prop_year
...
def calc_daly_annual(input_population, pars_country, start_year=2022, end_year=2030):
    ...
    for year in years:
        daly_benefit[year] = (
            sum(input_population[year, :])
            * pars_country["Social_value"][0]
...
def calc_YLD(
    pars_country, mat_morb, neo_morb, child_morb, start_year=2022, end_year=2030
):
    """
    ...
    child_dw = {
        "YLD": 1,
        "Stunting": 0.66,
        "Wasting": 0.128 * pars_country["Percent wasting severe"][0],
        "General": 0.10,
    }
    mat_dw_lifelong = {
        "YLD": False,
        ...
        "TB": True,
        "General": False,
    }  # Flags for whether these conditions are lifelong
    neo_dw_lifelong = {
        "YLD": False,
        "HIV": True,
        "Preterm": True,
    ...
    years = list(
        range(0, end_year - start_year + 1)
    )  # time vector for how many years of health impacts / data are entered
            ...
            for d, disability in enumerate(mat_morb.keys()):
                if year < len(mat_morb[disability]):
                    if not neo_dw_lifelong[disability]:
                        maternal_disability += (
                            mat_morb[disability][year] * mat_dw[disability]
                        )
        if bool(neo_morb):
            for d, disability in enumerate(neo_morb.keys()):
                if year < len(neo_morb[disability]):
                    if not neo_dw_lifelong[disability]:
                        neonatal_disability += (
                            neo_morb[disability][year] * neo_dw[disability]
                        )
        if bool(child_morb):
            for d, disability in enumerate(child_morb.keys()):
                if year < len(child_morb[disability]):
                    if not child_dw_lifelong[disability]:
                        child_disability += (
                            child_morb[disability][year] * child_dw[disability]
                        )
    ...
    for d, disability in enumerate(mat_morb.keys()):
        if mat_dw_lifelong[disability]:
            ...
            (
                ...
                p_neo_morbid,
                p_stillbirth_morbid,
                p_preg_averted_labour,
                p_preg_averted_education,
            ...
            maternal_disability_life = calc_daly_annual(
                p_mat_morbid * mat_dw[disability],
                pars_country,
                start_year=start_year,
                end_year=end_year,
            )
    ...
    for d, disability in enumerate(neo_morb.keys()):
        if neo_dw_lifelong[disability]:
            ...
            neonatal_disability_life = calc_daly_annual(
                p_neo_morbid * neo_dw[disability],
                pars_country,
                start_year=start_year,
                end_year=end_year,
            )
    ...
    for d, disability in enumerate(child_morb.keys()):
        if child_dw_lifelong[disability]:
            ...
            if disability == "Stunting":
                benefit_adjustment = child_dw[disability] * (
                    pars_country["GDP per capita"][0]
                    / pars_country["Average annual salary"][0]
                )
                child_disability_life = calc_work_benefits(
                    p_child_morbid * benefit_adjustment,
                    pars_country,
                ...
                child_disability_life = calc_daly_annual(
                    p_child_morbid * child_dw[disability],
                    pars_country,
                    start_year=start_year,
                    end_year=end_year,
                )
    ...
    daly = [
        ...
        for x in zip(
            daly_benefit,
            maternal_disability_life,
            child_disability_life,
            neonatal_disability_life,
        )
    ]
...
def calc_daly(
    ...
    years = list(
        range(1, len(mat_death) + 1)
    )  # time vector for how many years of health impacts / data are entered

    daly = []
    # Calculate discounted years of life lost from a single maternal, newborn or child death
    maternal_discount = sum(
        [
            1 / ((1 + pars_country["Discounting"][0]) ** t)
            for t in list(
                range(
                    0,
                    int(
                        pars_country["Expectation of life for women aged 25-29 years"][
                            0
                        ]
                        - pars_country["Average age of pregnancy"][0]
                    ),
                )
            )
        ]
    )
    neonatal_discount = sum(
        [
            1 / ((1 + pars_country["Discounting"][0]) ** t)
            for t in list(
                range(
    ...
    for year in years:
        ...
        stillbirth_YLL = (
            ...
            * neonatal_discount
            * (1 / ((1 + pars_country["Discounting"][0]) ** (year + 2)))
        )  # using proportion intrapartum
        total_YLL = (
            maternal_death_YLL
            + neonatal_death_YLL
            + children_death_YLL
            + stillbirth_YLL
        )  # Total YLL from deaths occurring in year
        ...
        if bool(
            mat_morb
        ):  # Check if maternal morbidity values are entered. mat_morb is a dictionary of vectors for difference morbidities
            for d, disability in enumerate(mat_morb.keys()):
                if mat_dw_lifelong[disability]:
                    maternal_disability += (
                        mat_morb[disability][year - 1]
                        * maternal_discount
                        * mat_dw[disability]
                        * (1 / ((1 + pars_country["Discounting"][0]) ** (year + 2)))
                    )
        ...
        daly_total = (
            total_YLL + total_YLD
        )  # add total YLL and YLD, attributed to outcomes in year
        daly.append(
            [
                neonatal_death_YLL,
                children_death_YLL,
